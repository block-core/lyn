...
namespace Lyn.Types.Serialization.Serializers
{
    ...
    {
        public NetworkAddressNoTime Deserialize(ref SequenceReader<byte> reader, ProtocolTypeSerializerOptions? options = null)
        {
            return new NetworkAddressNoTime { Services = reader.ReadULong(), IP = reader.ReadBytes(16).ToArray(), Port = reader.ReadUShort() };
        }

        public int Serialize(NetworkAddressNoTime typeInstance, IBufferWriter<byte> writer, ProtocolTypeSerializerOptions? options = null)
        {
            int size = 0;
